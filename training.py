...
img_size = int(len(X_train))
batch_size = 20
batch_len = int(img_size/batch_size)
train_data = np.array(X_train).reshape(batch_len,batch_size,-1)
train_label = np.array(y_train).reshape(batch_len,batch_size)
...
with tf.Session(config=tf.ConfigProto(gpu_options=(tf.GPUOptions(per_process_gpu_memory_fraction=0.7)))) as sess:
    ...
    for epoch in range(10):
        for i in range(batch_len):
            feed_cnn = {ipt_imgs: train_data[i], y_label: train_label[i]}
            loss_train,_ = sess.run([loss,optimizor],feed_dict=feed_cnn)
            
            if epoch == 9:
                saver.save(sess,'./model/model.ckpt',global_step = epoch)
            if epoch == 5:
                gc.collect()
            print("LOSS is:"+str(loss_train))
